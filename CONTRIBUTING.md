# Contributing to DotServe

First off, thanks for taking the time to contribute! 🎉

DotServe is built with Python/Flask backend and Alpine.js frontend. It's designed to be approachable — no Node.js build step, no complex toolchain. If you can run `python3 app.py`, you're ready to contribute.
...
### Reporting a bug
- Check [existing issues](https://github.com/khalequzzaman17/dotserve/issues) first
- Use the **Bug Report** issue template
- Include: OS and version, what you did, what happened, what you expected, any error messages from `/var/log/dotserve/error.log`
...
Panel runs at `http://127.0.0.1:3334`. Admin password is printed on first run and saved to `admin_password.txt`.
...
- Every page component's `init()` must include: `document.addEventListener('dotserve-logged-in', () => { this.init(); });`
...
5. Add an `async init()` with the `dotserve-logged-in` listener
