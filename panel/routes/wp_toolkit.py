...
# --- Paths ----------------------------------------------------------------------
WP_BACKUP_DIR = '/opt/dotserve/wp_backups'
...
def _delete_vhost(domain, webserver):
    ws = webserver or _detect_webserver()
    if ws == 'nginx':
        for p in [f'/etc/nginx/dotserve/{domain}.conf', f'/etc/nginx/conf.d/{domain}.conf']:
            try: os.unlink(p)
            except: pass
        sh('systemctl reload nginx 2>/dev/null')
    elif ws == 'apache':
...
def _scan_wp_sites():
    ...
    for conf_dir in ['/etc/nginx/dotserve', '/etc/nginx/conf.d', '/etc/apache2/sites-enabled']:
...
def install_wp():
    ...
    if system_cron:
        ...
        cron_file = f'/etc/cron.d/dotserve-wp-{re.sub(chr(46), "_", domain)}'
        try:
...
def clone_site(domain):
    ...
    if clone_type in ('full', 'db') and _wp_installed():
        ...
        # Export source DB
        dump_file = f'/tmp/dotserve_clone_{domain}.sql'
...
def push_staging(domain):
    ...
    if _wp_installed():
        ...
        dump = f'/tmp/dotserve_push_{domain}.sql'
