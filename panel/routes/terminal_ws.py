...
import os, struct, subprocess, threading, signal, time
try:
    import pty, fcntl, termios, select
    _PTY_AVAILABLE = True
except ImportError:
    pty = fcntl = termios = select = None
...
def terminal_ws(ws):
    ...
    if not _PTY_AVAILABLE:
        ws.send('Terminal websocket requires a Unix-like PTY environment and is unavailable on this host.\r\n')
        ws.close()
        return
