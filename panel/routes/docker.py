...
from panel.core.validation import valid_docker_image, valid_port, valid_safe_name
...
DOCKER_DOMAINS_FILE = '/opt/dotserve/docker_domains.json'
...
def container_action(cid):
    if not req(): return jsonify({'ok': False}), 401
    if not valid_safe_name(cid):
        return jsonify({'ok': False, 'error': 'Invalid container id/name'}), 400
    action = (request.get_json() or {}).get('action', '')
    if action not in ('start','stop','restart','remove','pause','unpause'):
        return jsonify({'ok': False, 'error': 'Invalid action'}), 400
    args = ['docker', 'rm', '-f', cid] if action == 'remove' else ['docker', action, cid]
    _, err, rc = run_args(args, timeout=30)
...
def container_logs(cid):
    ...
    try:
        ...
        lines = 100
    out, err, _ = run_args(['docker', 'logs', '--tail', str(lines), cid], timeout=30)
    out = out or err
...
def container_stats(cid):
    ...
    if not valid_safe_name(cid):
        return jsonify({'ok': False, 'error': 'Invalid container id/name'}), 400
    out, _, rc = run_args(['docker', 'stats', cid, '--no-stream', '--format', '{{json .}}'], timeout=15)
...
def remove_image(image_id):
    if not req(): return jsonify({'ok': False}), 401
    if not valid_docker_image(image_id):
        return jsonify({'ok': False, 'error': 'Invalid image id/name'}), 400
    _, err, rc = run_args(['docker', 'rmi', image_id], timeout=60)
...
def pull_image():
    ...
    if not valid_docker_image(image):
        return jsonify({'ok': False, 'error': 'Invalid Docker image reference'}), 400
...
def run_container():
    ...
    if not valid_docker_image(image):
        return jsonify({'ok': False, 'error': 'Invalid Docker image reference'}), 400
    if name and not valid_safe_name(name):
        return jsonify({'ok': False, 'error': 'Invalid container name'}), 400
    ...
    display_parts = ['docker', 'run', '-d']
    if name:
        docker_args += ['--name', name]
        display_parts += ['--name', name]
    if restart:
        docker_args.append(f'--restart={restart}')
        display_parts.append(f'--restart={restart}')
    for p in ports:
        if p.get('host') and p.get('container'):
            if not valid_port(p['host']) or not valid_port(p['container']):
                return jsonify({'ok': False, 'error': 'Invalid port mapping'}), 400
            mapping = f'{p["host"]}:{p["container"]}'
            docker_args += ['-p', mapping]
            display_parts += ['-p', mapping]
    for e in envs:
        if e.get('key') and e.get('value') is not None:
            if not valid_safe_name(e['key']):
                return jsonify({'ok': False, 'error': f'Invalid environment key: {e["key"]}'}), 400
            env_pair = f'{e["key"]}={e["value"]}'
            docker_args += ['-e', env_pair]
            display_parts += ['-e', f'{e["key"]}=***']
    for v in volumes:
        if v.get('host') and v.get('container'):
            os.makedirs(v['host'], exist_ok=True)
            volume = f'{v["host"]}:{v["container"]}'
            docker_args += ['-v', volume]
            display_parts += ['-v', volume]
    if network:
        if not valid_safe_name(network):
            return jsonify({'ok': False, 'error': 'Invalid network name'}), 400
        docker_args.append(f'--network={network}')
        display_parts.append(f'--network={network}')
    docker_args.append(image)
    display_parts.append(image)
    if cmd_extra:
        docker_args += str(cmd_extra).split()
        display_parts.append(str(cmd_extra))

    full_cmd = ' '.join(display_parts)
    job_id   = str(uuid.uuid4())[:8]
    ...
    def run():
        ...
        out, err, rc = run_args(docker_args, timeout=60)
        if rc == 0:
            cid = out.strip()[:12]
