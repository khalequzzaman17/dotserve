...
# Previously DotServe could only view/ban/unban IPs on jails that already
...
VORTEX_SITE_PREFIX   = 'dotserve-site-'
VORTEX_SERVER_PREFIX = 'dotserve-server-'
    ...
    if os.path.isdir(F2B_JAIL_DIR):
        for fname in sorted(os.listdir(F2B_JAIL_DIR)):
            ...
            for section, opts in cfg.items():
                ...
                jails.append({
                    'name': section,
                    'site': opts.get('_dotserve_site', ''),
                    'port': opts.get('port', ''),
                    'mode': opts.get('_dotserve_mode', 'anti-cc'),
...
def create_website_jail():
    ...
    jail_content = (
        ...
        f'_dotserve_site = {site}\n'
        f'_dotserve_mode = {mode}\n'
    )
...
def security_score():
    ...
    try:
        ...
        creds_file = '/opt/dotserve/credentials.json'
    ...
    checks.append({'label':'Panel Secret Key Auto-Generated (not default)',
                   'pass': os.path.exists('/opt/dotserve/secret.key'),
                   'severity':'high'})
...
MODSEC_LISTS_CONF = '/etc/nginx/modsec/dotserve-lists.conf'
MODSEC_LISTS_JSON = '/etc/nginx/modsec/dotserve-lists.json'
...
def _render_lists_conf(data):
    ...
    lines = ['# Auto-generated by DotServe — do not edit by hand, use the WAF Blacklist/Whitelist page',
    ...
    for i, ip in enumerate([x.strip() for x in data.get('ip_blacklist', []) if x.strip()]):
        ...
        lines.append(
            ...
            f'"id:{rid},phase:1,deny,status:403,log,msg:\'DotServe IP Blacklist\'"'
        )
    ...
    for i, ua in enumerate([x.strip() for x in data.get('ua_blacklist', []) if x.strip()]):
        ...
        lines.append(
            ...
            f'"id:{rid},phase:1,deny,status:403,log,msg:\'DotServe UA Blacklist\'"'
        )
    ...
    for i, url in enumerate([x.strip() for x in data.get('url_blacklist', []) if x.strip()]):
        ...
        lines.append(
            ...
            f'"id:{rid},phase:1,deny,status:403,log,msg:\'DotServe URL Blacklist\'"'
        )

    return '\n'.join(lines) + '\n'

def _ensure_lists_included():
    """Insert the Include for dotserve-lists.conf right after modsecurity.conf
...
def modsec_status():
    ...
    site_overrides = {}
    for conf_dir in ['/etc/nginx/dotserve', '/etc/nginx/conf.d']:
...
def lb_save():
    ...
    conf = f"""# DotServe Load Balancer — managed by DotServe
...
LB_STREAM_CONF = '/etc/nginx/stream.d/dotserve_tcp_lb.conf'
...
def lb_tcp_save():
    ...
    conf = f"""# DotServe TCP Load Balancer — managed by DotServe
# Method: {method}
upstream dotserve_tcp_backend {{
{method_directive}{server_lines}
}}

server {{
    listen {port_n};
    proxy_pass dotserve_tcp_backend;
    proxy_timeout 10m;
...
LB_HEALTH_CONFIG = '/opt/dotserve/lb_health.json'
LB_HEALTH_STATE  = '/opt/dotserve/lb_health_state.json'
LB_HEALTH_LOG    = '/opt/dotserve/lb_health.log'
LB_HEALTH_SCRIPT = '/opt/dotserve/scripts/lb_healthcheck.py'
LB_HEALTH_SERVICE_FILE = '/etc/systemd/system/dotserve-lb-healthcheck.service'
LB_HEALTH_SERVICE_NAME = 'dotserve-lb-healthcheck'

_HEALTHCHECK_SCRIPT_BODY = '''#!/usr/bin/env python3
"""
DotServe Load Balancer — active health check daemon.
...
CONFIG  = "/opt/dotserve/lb_health.json"
STATE   = "/opt/dotserve/lb_health_state.json"
LOG     = "/opt/dotserve/lb_health.log"
LB_CONF = "/etc/nginx/conf.d/loadbalancer.conf"
...
def check_http(address, path, timeout):
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "DotServe-HealthCheck"})
    ...
    for line in content.split("\\n"):
        ...
        if m:
            ...
            if is_healthy and was_commented:
                ...
                new_lines.append(indent + "#server " + addr + weight + "; # DotServe: marked unhealthy")
                changed = True
            else:
...
def _install_health_service():
    ...
    service = f"""[Unit]
Description=DotServe Load Balancer Active Health Check
...
def lb_health_save():
    ...
    if cfg['enabled']:
        ...
        if os.path.exists(LB_CONF):
            content = open(LB_CONF).read()
            restored = re.sub(r'#server ([^\s;{}]+)(\s+weight=\d+)?; # DotServe: marked unhealthy',
