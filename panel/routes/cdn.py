...
CONFIG_FILE = '/opt/dotserve/cdn_config.json'
...
def generic_test():
    ...
    try:
        ...
        req2.add_header('User-Agent', 'DotServe/1.0 CDN-Test')
...
def apply_nginx_headers():
    ...
    nginx_snippet = f"""
    # DotServe CDN: {provider} cache headers for {domain}
...
"""
    ...
    # Test and reload
    out, err, rc = run_args(['nginx', '-t'], timeout=20)
    if rc != 0:
        return jsonify({'ok':False,'error':f'Nginx error: {out or err}'}), 400
    _, _, rc = run_args(['systemctl', 'reload', 'nginx'], timeout=20)
    if rc != 0:
        run_args(['nginx', '-s', 'reload'], timeout=20)
