"""
DotServe HTTP/3 (QUIC) support — multi-webserver aware.
...
Per-webserver reality:
  ...
  LiteSpeed Enterprise          — not supported by DotServe (paid/closed-source)
"""
import os, re, shutil
    ...
    if ws == 'apache':
        return jsonify({
            ...
            'message': 'Apache does not have native HTTP/3 support. To use HTTP/3 with Apache, '
                       ...
                       'not currently managed by DotServe.',
            'enabled': False,
        })
    ...
    if ws == 'apache':
        return jsonify({
            ...
            'error': 'Apache does not support HTTP/3 natively. '
                     'DotServe does not manage the nginx-in-front workaround.',
        }), 400
