...
old panel, DotServe extracts it, makes a best-effort detection of the
...
Site files and databases are restored directly. Email, cron, SSL, and DNS
artifacts are detected and preserved so migration can be completed safely per
server environment.
"""
...
IMPORT_WORKSPACE = '/opt/dotserve/import_workspace'
        ...
        if lower.endswith('.zip'):
            with zipfile.ZipFile(archive_path) as zf:
                for member in zf.infolist():
                    if not safe_target(member.filename):
                        return False, f'Unsafe archive path rejected: {member.filename}'
                zf.extractall(root)
            ...
            with tarfile.open(archive_path, mode) as tf:
                for member in tf.getmembers():
                    if member.issym() or member.islnk():
                        return False, f'Archive links are not allowed: {member.name}'
                    if not safe_target(member.name):
                        return False, f'Unsafe archive path rejected: {member.name}'
                tf.extractall(root)
        else:
...
def _mysql_base_args():
    args = ['mysql', '-u', 'root']
    for sock in ['/var/run/mysqld/mysqld.sock', '/run/mysqld/mysqld.sock']:
        if os.path.exists(sock):
            args.append(f'--socket={sock}')
            break
    return args


def _run_mysql(sql, timeout=30):
    return subprocess.run(_mysql_base_args() + ['-e', sql],
                          capture_output=True, text=True, timeout=timeout)


def _import_mysql_dump(dump_path, database):
    opener = gzip.open if dump_path.endswith('.gz') else open
    with opener(dump_path, 'rb') as fh:
        return subprocess.run(_mysql_base_args() + [database],
...
def _detect_extended_artifacts(extract_dir, info):
    artifacts = {'email': [], 'cron': [], 'ssl': [], 'dns': []}
    for root, _, files in os.walk(extract_dir):
        root_l = root.lower().replace('\\', '/')
        for fname in files:
            lower = fname.lower()
            fp = os.path.join(root, fname)
            if '/mail' in root_l or lower in ('passwd', 'shadow', 'aliases'):
                artifacts['email'].append(fp)
            if 'cron' in lower or '/cron' in root_l:
                artifacts['cron'].append(fp)
            if lower.endswith(('.crt', '.key', '.pem', '.csr')) or '/ssl' in root_l:
                artifacts['ssl'].append(fp)
            if lower.endswith(('.zone', '.db')) or '/dns' in root_l:
                artifacts['dns'].append(fp)
    info['artifacts'] = {k: v[:50] for k, v in artifacts.items()}
    for key, files in info['artifacts'].items():
        if files:
            info['notes'].append(f'Detected {len(files)} {key.upper()} migration artifact(s)')
...
def _detect_cpanel(extract_dir):
    ...
    _detect_extended_artifacts(extract_dir, info)
    return info
...
def execute_import(import_id):
    ...
    import_options = d.get('import_options') or {}
    ...
    def run_job():
        try:
            _job_append(job_id, f'[DotServe] Starting import for {domain}')

            # 1. Create the site (reusing the exact same core logic as "New Website")
            _job_append(job_id, '[DotServe] Creating website + nginx vhost...')
            ...
            if not ok:
                ...
                return
            _job_append(job_id, f'[DotServe] ✓ Site created: {site_path}')

            # 2. Copy site files from the extracted doc root into the new webroot
            _job_append(job_id, f'[DotServe] Copying files from {doc_root} ...')
            try:
                _copy_tree_contents(doc_root, site_path)
            except Exception as e:
                _job_append(job_id, f'[ERROR] File copy failed: {str(e)[:300]}')
                _job_finish(job_id, False)
                return
            _job_append(job_id, f'[DotServe] ✓ Copied {_file_count(site_path)} files')

            ensure_web_ownership(site_path)
            _job_append(job_id, '[DotServe] ✓ File ownership set for web server user')
            ...
            for db in databases:
                ...
                _job_append(job_id, f'[DotServe] Creating database `{target_name}`...')
                create_out = _run_mysql(
                    f'CREATE DATABASE IF NOT EXISTS `{target_name}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;',
                    timeout=30
                )
                ...
                _run_mysql(
                    f"CREATE USER IF NOT EXISTS '{target_name}'@'localhost' IDENTIFIED BY '{new_password}'; "
                    f"GRANT ALL PRIVILEGES ON `{target_name}`.* TO '{target_name}'@'localhost'; FLUSH PRIVILEGES;",
                    timeout=30
                )

                _job_append(job_id, f'[DotServe] Importing dump ({os.path.basename(dump_path)})...')
                imp = _import_mysql_dump(dump_path, target_name)
                if imp.returncode != 0:
                    stderr = imp.stderr.decode(errors='replace') if isinstance(imp.stderr, bytes) else (imp.stderr or '')
                    _job_append(job_id, f'[ERROR] Import failed for {target_name}: {stderr.strip()[:300]}')
                    continue

                _job_append(job_id, f'[DotServe] ✓ Database `{target_name}` imported — user `{target_name}` / password: {new_password}')
                imported_dbs.append({'name': target_name, 'user': target_name, 'password': new_password})

            _job_append(job_id, f'[DotServe] ✓ Import complete for {domain}')
            if imported_dbs:
                _job_append(job_id, '[DotServe] IMPORTANT: save these generated database credentials — they will not be shown again:')
                for db in imported_dbs:
                    _job_append(job_id, f'   • {db["name"]} — user: {db["user"]} — password: {db["password"]}')

            for artifact in ('email', 'cron', 'ssl', 'dns'):
                if import_options.get(artifact):
                    _job_append(job_id, f'[DotServe] {artifact.upper()} migration requested; detected artifacts remain in the import workspace for review.')
