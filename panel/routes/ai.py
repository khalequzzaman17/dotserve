...
CONFIG_FILE = '/opt/dotserve/ai_config.json'

# Default DotServe AI config
DEFAULT_CONFIG = {
    'enabled':    True,
    'api_key':    '',
...
SYSTEM_PROMPT = """You are DotServe AI Assistant.
You are a server management expert integrated directly into DotServe — a Linux server control panel.
...
def list_models():
    if not req(): return jsonify({'ok': False}), 401
    cfg = load_config()
    # Try to fetch models from the configured API
    try:
        ...
        return jsonify({'ok': True, 'models': [
            {'id': 'dotserve-default', 'name': 'DotServe Default'},
        ], 'error': str(e)})
...
def chat():
    ...
    if not cfg.get('api_key'):
        return jsonify({'ok': False, 'error': 'AI API key not configured. Go to Settings → AI Assistant to set it up.'}), 400
...
def quick_action():
    ...
    cfg = load_config()
    if not cfg.get('api_key'):
        return jsonify({'ok': False, 'error': 'AI API key not configured.'}), 400
