...
WEBROOT = '/www/wwwroot'
CF_CONFIG_FILE = '/opt/dotserve/cdn_config.json'
INTEGRITY_DIR = '/opt/dotserve/integrity'
...
def get_nginx_dirs():
    """Return DotServe-managed nginx vhost directory"""
    dotserve_dir = '/etc/nginx/dotserve'
    os.makedirs(dotserve_dir, exist_ok=True)
    ...
    if os.path.exists(nginx_conf):
        ...
        if 'dotserve' not in nc:
            import subprocess as _sp
            _sp.run("sed -i 's|include /etc/nginx/conf.d/\\*.conf;|include /etc/nginx/conf.d/*.conf;\\n    include /etc/nginx/dotserve/*.conf;|' " + nginx_conf, shell=True)
    return dotserve_dir, dotserve_dir
...
def create_site_core(domain, path=None, php='8.3'):
    ...
    if not os.path.exists(idx):
        with open(idx, 'w') as f:
            f.write(f'<!DOCTYPE html><html><body><h1>Welcome to {domain}</h1><p>DotServe — site created successfully.</p></body></html>')
