...
CREDS_LOCATIONS = [
    '/opt/dotserve/credentials.json',
    '/opt/dotserve/config/credentials.json',
    '/etc/dotserve/credentials.json',
    '/root/.dotserve/credentials.json',
]
CREDS_FILE   = '/opt/dotserve/credentials.json'
AUDIT_FILE   = '/opt/dotserve/login_audit.log'
CONFIG_FILE  = '/opt/dotserve/config.json'
LOCKOUT_FILE = '/opt/dotserve/lockout.json'
...
def _ip_allowed(ip):
    if ip in _ALWAYS_ALLOWED:
        return True
    if os.environ.get('DOTSERVE_DISABLE_IP_CHECK'):
        return True
    try:
...
def setup_2fa():
    ...
    uri    = _pyotp.TOTP(secret).provisioning_uri(
        name=session.get('user', 'admin'), issuer_name='DotServe')
