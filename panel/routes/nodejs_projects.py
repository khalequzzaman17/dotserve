"""
Node.js Project Manager for DotServe
...
def svc_name(pid): return f'dotserve-node-{pid}'
...
def write_proxy_conf(p):
    ...
    listen 80;
    server_name {all_d};
    access_log /var/log/nginx/dotserve-node-{pid}-access.log;
    error_log  /var/log/nginx/dotserve-node-{pid}-error.log;
...
"""
        conf_path = f'/etc/nginx/conf.d/dotserve-node-{pid}.conf'
    ...
    ErrorLog {log_dir}/dotserve-node-{pid}-error.log
    CustomLog {log_dir}/dotserve-node-{pid}-access.log combined
</VirtualHost>
"""
        conf_dir  = apache_conf_dir()
        conf_name = f'dotserve-node-{pid}'
        ...
        vhost_dir = f'/usr/local/lsws/conf/vhosts/dotserve-node-{pid}'
        os.makedirs(vhost_dir, exist_ok=True)
        conf = f"""docRoot                   /var/www/html
virtualHostConfig  {{
  extprocessor dotserve-node-{pid} {{
    type                    proxy
...
"""
    ...
    log {{
        output file /var/log/caddy/dotserve-node-{pid}.log
    }}
}}
"""
        conf_path = f'/etc/caddy/sites/dotserve-node-{pid}.caddy'
        open(conf_path, 'w').write(conf)
...
def remove_proxy_conf(pid):
    ...
    sh(f'rm -f /etc/nginx/conf.d/dotserve-node-{pid}.conf 2>/dev/null')

    # Apache — clean BOTH Debian and RHEL paths
    sh(f'a2dissite dotserve-node-{pid} 2>/dev/null || true')
    sh(f'rm -f /etc/apache2/sites-available/dotserve-node-{pid}.conf 2>/dev/null')
    sh(f'rm -f /etc/apache2/sites-enabled/dotserve-node-{pid}.conf 2>/dev/null')
    sh(f'rm -f /etc/httpd/conf.d/dotserve-node-{pid}.conf 2>/dev/null')

    # OLS (same path on all distros)
    sh(f'rm -rf /usr/local/lsws/conf/vhosts/dotserve-node-{pid}/ 2>/dev/null')

    # Caddy (same path on all distros)
    sh(f'rm -f /etc/caddy/sites/dotserve-node-{pid}.caddy 2>/dev/null')
...
def write_systemd_node(p):
    ...
    unit = f"""[Unit]
Description=DotServe Node.js: {p['name']}
