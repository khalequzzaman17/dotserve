...
CONFIG_FILE  = '/opt/dotserve/config.json'
SSL_DIR      = '/opt/dotserve/ssl'
SERVICE_FILE = '/etc/systemd/system/dotserve.service'
PANEL_PORT   = 3334
...
def load_config():
    ...
    return {'panel_name':'DotServe','port':3334,
...
def _safe_restart_panel():
    """
    Restart dotserve.service from WITHIN a request handled by that very
    service. A naive '(sleep 2 && systemctl restart dotserve) &' is
    unsafe: that background process is still a member of dotserve's
    ...
    *transient* unit, outside dotserve's cgroup, so it survives the kill
    ...
    if sh('which systemd-run 2>/dev/null'):
        sh('systemd-run --no-block --collect --unit=dotserve-restart '
           '/bin/sh -c "sleep 2 && systemctl restart dotserve" 2>/dev/null')
    elif sh('which setsid 2>/dev/null'):
        sh('setsid sh -c "sleep 2 && systemctl restart dotserve" '
           '>/dev/null 2>&1 < /dev/null &')
    else:
        sh('(sleep 2 && systemctl restart dotserve) >/dev/null 2>&1 &')
...
def _gen_selfsigned(domain=''):
    ...
    out, err, rc = sh3(
        ...
        f'-subj "/CN={cn}/O=DotServe/OU=Panel" '
...
def change_port():
    ...
    if not req(): return jsonify({'ok':False}), 401
    new_port = int((request.get_json() or {}).get('port', 3334))
    if not (1024 <= new_port <= 65535):
        return jsonify({'ok':False,'error':'Port must be 1024–65535'}), 400
    cfg = load_config()
    old_port = cfg.get('port', 3334)
...
def ssl_disable():
    ...
    nginx_ssl = '/etc/nginx/conf.d/dotserve-https.conf'
