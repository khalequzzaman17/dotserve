...
MAXC=$(( TOTAL_MB / 80 )) && \
[ $MAXC -lt 10 ] && MAXC=10; [ $MAXC -gt 150 ] && MAXC=150 && \
STARTS=$(( MAXC / 6 )); [ $STARTS -lt 2 ] && STARTS=2 && \
MINSP=$(( STARTS )); MAXSP=$(( MAXC / 3 )); [ $MAXSP -lt $MINSP ] && MAXSP=$MINSP && \
sed -i "s|^pm\\.max_children.*|pm.max_children = $MAXC|" $POOL && \
sed -i "s|^pm\\.start_servers.*|pm.start_servers = $STARTS|" $POOL && \
sed -i "s|^pm\\.min_spare_servers.*|pm.min_spare_servers = $MINSP|" $POOL && \
sed -i "s|^pm\\.max_spare_servers.*|pm.max_spare_servers = $MAXSP|" $POOL && \
...
apt-get autoremove -y 2>/dev/null || true''',
    ...
    # --- Admin Tools -----------------------------------------------------------
    {
        ...
        'install':(
            ...
            'echo "[DotServe] phpMyAdmin ready at http://YOUR-SERVER-IP:8082"'
        ),
        'uninstall':(
...
chown -R www-data:www-data /var/www/roundcube/''',
    ...
    # --- WAF / Security ---------------------------------------------------------
    {
        ...
        'install_tpl':(
            ...
            # on UBUNTU'S OWN nginx build. DotServe installs nginx from nginx.org's
            ...
            'OS_FAMILY=$(. /etc/os-release 2>/dev/null && echo $ID_LIKE || echo debian); '
            'echo "[DotServe] Installing ModSecurity engine..."; '
            ...
            '    echo "[DotServe] Compiling nginx-ModSecurity connector for nginx $NGINX_VER..."; '
            ...
            '      CONNECTOR_OK=1; '
            '      echo "[DotServe] ✓ Connector compiled for nginx $NGINX_VER — WAF can actually load in nginx"; '
            '    else '
            ...
            'echo "[DotServe] Writing core engine config..."; '
            ...
            '  echo "[DotServe] modsecurity.conf download attempt $attempt failed, retrying..."; sleep 2; '
            'done; '
            'if [ "$CONF_OK" = "1" ]; then '
            ...
            '  echo "[DotServe] ✓ Core engine config written — Engine Mode toggle will work"; '
            'else '
            ...
            'echo "[DotServe] Downloading OWASP CRS ruleset..."; '
            ...
            '  echo "[DotServe] CRS download attempt $attempt failed, retrying..."; sleep 3; '
            'done; '
            ...
            '  echo "[DotServe] ✓ OWASP CRS $CRS_TAG installed — Paranoia level control will work"; '
            'else '
            ...
            'else '
            '  echo "[DotServe] Skipping nginx.conf changes — connector module isn'"'"'t present. nginx stays working; WAF stays inactive until the connector build succeeds."; '
            'fi; '
            ...
            'rm -f /tmp/crs.tar.gz && '
            'nginx -t && systemctl reload nginx\\"" > /etc/cron.d/dotserve-crs-update && '
            'chmod 644 /etc/cron.d/dotserve-crs-update; '
            ...
            '  echo "[DotServe] ✓ nginx config test passed — WAF is actually serving traffic"; '
            'else '
            ...
            '    nginx -t 2>&1 && systemctl reload nginx 2>/dev/null && echo "[DotServe] ✓ nginx.conf restored, server is back up" '
            '      || echo "[ERROR] Restore also failed nginx -t — nginx.conf may have been broken before this install ran too. Manual check required."; '
            '  fi; '
            'fi; '
            'echo "[DotServe] ModSecurity install finished. Connector: $([ \\"$CONNECTOR_OK\\" = \\"1\\" ] && echo compiled-and-enabled || echo FAILED — WAF NOT active, see /tmp/modsec-connector-*.log). Engine: $([ \\"$CONF_OK\\" = \\"1\\" ] && echo ready || echo fallback-config). CRS ruleset: $([ \\"$CRS_OK\\" = \\"1\\" ] && echo loaded || echo MISSING — use Repair CRS on the WAF page)."'
        ),
        'uninstall':(
            'OS_FAMILY=$(. /etc/os-release 2>/dev/null && echo $ID_LIKE || echo debian); '
            'if echo "$OS_FAMILY" | grep -qiE "debian|ubuntu"; then '
            ...
            'rm -rf /etc/nginx/modsec /etc/cron.d/dotserve-crs-update && '
...
# DotServe Load Balancer Configuration
# Method: {ver}
# Edit upstream servers below to match your backend servers

upstream dotserve_backend {{
    # {ver} load balancing
...
# upstream dotserve_backend {{ least_conn; server ...; }}

# Uncomment to use IP Hash (sticky sessions):
# upstream dotserve_backend {{ ip_hash; server ...; }}
...
server {{
    ...
    location / {{
        proxy_pass http://dotserve_backend;
...
nginx -t && systemctl reload nginx''',
    ...
    # --- CDN --------------------------------------------------------------------
    {
        ...
        'install':'mkdir -p /opt/dotserve && echo "{}" > /opt/dotserve/cdn_config.json',
        'uninstall':'rm -f /opt/dotserve/cdn_config.json',
        'manage':False,
    },
]
...
def install_module(mod_id):
    ...
    def run_job():
        _job_append_line(job_id, f'[DotServe] Installing {mod["name"]} {ver}...')
        ...
        for line in proc.stdout:
            ...
            if _time.time() - start > MAX_SECONDS:
                proc.kill()
                _job_append_line(job_id, '[DotServe] ⚠ Timed out after 10 minutes. Process killed.')
                break
        proc.wait()
        ...
        _job_append_line(job_id,
            f'[DotServe] {"✓ Installed successfully! Version: "+inst_ver if installed else "⚠ Installation may have failed — check output above."}'
        )
...
def uninstall_module(mod_id):
    ...
    def run_job():
        _job_append_line(job_id, f'[DotServe] Removing {mod["name"]} {ver}...')

        # Stop the service first to prevent dpkg from hanging on restart triggers
        svc = mod.get('service', mod_id)
        if svc:
            _job_append_line(job_id, f'[DotServe] Stopping {svc} service...')
        ...
        for line in proc.stdout:
            ...
            if _time.time() - start > MAX_SECONDS:
                proc.kill()
                _job_append_line(job_id, '[DotServe] ⚠ Timed out after 5 minutes. Process killed.')
                break
        proc.wait()
        ...
        _job_append_line(job_id,
            f'[DotServe] {"✓ Removed successfully!" if removed else "⚠ May not be fully removed — check output above."}'
        )
        panel_cache.invalidate('modules_list')
...
def ffmpeg_install_version(version):
    ...
    def run_job():
        try:
            _job_append_line(job_id, f'[DotServe] FFmpeg {version} — {arch} build')
            _job_append_line(job_id, f'[DotServe] Source: BtbN/FFmpeg-Builds (listed on ffmpeg.org/download.html#build-linux)')
            _job_append_line(job_id, f'[DotServe] Downloading: {url}')
            ...
            _job_append_line(job_id, f'[DotServe] Downloaded {round(file_size/1024/1024, 1)} MB — extracting...')
            ...
            if ext.returncode != 0:
                ...
                _job_append_line(job_id, '[DotServe] Cleaned up partial install directory')
                _job_finish(job_id, False, False)
                return
            ...
            _job_append_line(job_id, f'[DotServe] ✓ Verified: {ver_line}')
            _job_append_line(job_id, f'[DotServe] ✓ Installed to {dest_dir}/bin/ffmpeg')
            _job_append_line(job_id, f'[DotServe] ✓ Command alias: {v["alias"]} -> {binary}')
...
def get_module_settings(mod_id):
    ...
    if mod_id == 'nginx':
        ...
        cfg_file = '/opt/dotserve/ddns_config.json'
        cfg = {}
        if os.path.exists(cfg_file):
            try:
        ...
        log = ''
        log_file = '/opt/dotserve/ddns.log'
...
def save_module_settings(mod_id):
    ...
    if action == 'save_config':
        ...
        def run_switch():
            ...
            _job_append_line(job_id, f'[DotServe] Switching {mod_name} to version {ver}...')
            env = os.environ.copy()
            env['DEBIAN_FRONTEND'] = 'noninteractive'
            ...
            for line in proc.stdout:
                ...
                if _time.time() - start > MAX_SECONDS:
                    proc.kill()
                    _job_append_line(job_id, '[DotServe] ⚠ Timed out after 8 minutes. Operation killed.')
                    break
            proc.wait()
            success = proc.returncode == 0
            new_ver = sh(ver_check_cmd) if ver_check_cmd else ver
            _job_append_line(job_id,
                f'[DotServe] {"✓ Switched to " + new_ver + " successfully!" if success else "⚠ Switch failed — check output above."}'
            )
        ...
        # Export Caddy certs to /etc/ssl/dotserve/ for portability
        cert_dirs = [
        ...
        for base in cert_dirs:
            ...
            for domain in domains:
                ...
                dest = f'/etc/ssl/dotserve/{domain}'
                sh(f'mkdir -p {dest}')
