#!/usr/bin/env python3
"""DotServe v1.0.0 - Main Application"""
import os, sys, secrets
from datetime import timedelta
sys.path.insert(0, os.path.dirname(__file__))

from flask import Flask, request, g, jsonify
from panel.core.config import DEFAULT_PORT, SECRET_KEY_FILE, SESSION_DIR, ensure_data_dirs
from panel.core.security import apply_security_headers
try:
...
from panel.routes.alerting import alerting_bp
from panel.routes.disk_usage import disk_usage_bp
from panel.routes.onboarding import onboarding_bp
from panel.routes.webshell import webshell_bp
...
def create_app():
    ...
    app.config['SESSION_COOKIE_NAME']        = 'dotserve_session'
    app.config['SESSION_REFRESH_EACH_REQUEST'] = True
    if os.environ.get('DOTSERVE_COOKIE_SECURE', '').lower() in ('1', 'true', 'yes'):
    ...
    for bp in [auth_bp, dashboard_bp, websites_bp, databases_bp, files_bp,
               ...
               caddy_bp, cdn_bp, update_bp, ddns_bp, cloud_backup_bp,
               logs_bp, wp_bp, nodejs_bp, go_bp, import_bp, alerting_bp,
               disk_usage_bp, onboarding_bp, webshell_bp]:
        ...
        if request.method in ('POST', 'PUT', 'PATCH', 'DELETE'):
            ...
            if origin:
                ...
                if parsed.netloc and parsed.netloc != request.host:
                    return jsonify({'ok': False, 'error': 'Cross-origin API request denied'}), 403
