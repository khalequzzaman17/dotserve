...
function rootApp() {
  return {
    ...
    updateModal: {
      show:false, current:'v1.0.0', latest:'', name:'',
    ...
    nav: [
      ...
      { id: 'network', group: 'Network', items: [
      ...
      { id: 'system', group: 'System', items: [
        ...
        { id:'alerts',     icon:'!', label:'Alerts',      color:'#e11d48', colorBg:'#ffe4e6' },
        { id:'disk-usage', icon:'D', label:'Disk Usage',  color:'#ca8a04', colorBg:'#fef9c3' },
        { id:'webshell',   icon:'P', label:'PHP Scanner', color:'#be123c', colorBg:'#ffe4e6' },
        { id:'onboarding', icon:'?', label:'Onboarding',  color:'#0f766e', colorBg:'#ccfbf1' },
    ...
    async setLang(code) {
      if (!window.SUPPORTED_LANGS || !window.SUPPORTED_LANGS[code]) code = 'en';
    ...
    async silentUpdateCheck() {
      try {
        ...
        if (r.ok && r.has_update) {
          ...
          this.updateModal.name      = r.name      || 'DotServe';
    ...
    async openUpdateModal() {
      ...
      this.$nextTick(() => {
        document.dispatchEvent(new CustomEvent('dotserve-check-update'));
      });
    },
  };
}
...
function dashboardPage() {
  return {
    ...
    async init() {
      ...
      document.addEventListener("dotserve-logged-in", () => { this.init(); });
...
function websitesPage() {
  return {
    ...
    async init() {
      ...
      await this.load();
      document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="websites") this.load(); });
    },
...
function aiAssistant() {
  return {
    ...
    async init() {
      ...
      document.addEventListener('dotserve-toggle-ai', () => {
        this.open = !this.open;
      ...
      if (r.ok) {
        ...
        this.modelName  = r.config.model || 'DotServe AI';
      }
    },
    ...
    async send() {
      ...
      if (!this.configured) {
        toast('Set your AI API key in Settings → AI Assistant first', 'error');
        return;
      }
      this.input = '';
    ...
    async sendQuick(q) {
      if (!this.configured) {
        toast('Set your AI API key in Settings first', 'error');
        return;
      }
...
function settingsPage() {
  return {
    ...
    panelVersion: 'v3.2.0',
    cfg: {panel_name:'DotServe', port:3334, ssl_enabled:false, auto_update:true, timezone:'UTC', panel_domain:'', security_path:''},
    ...
    async loadSettings() {
      ...
      this.newPort = String(this.cfg.port||3334);
    ...
    async pollSslApply(type, targetScheme) {
      ...
      const port = this.cfg.port || 3334;
      const host = window.location.hostname;
      const targetUrl = `${targetScheme}://${host}:${port}/#settings`;

      if (definitiveResult) {
        const success = (definitiveResult.log||'').includes('dotserve=active') &&
    ...
    async testAiConnection() {
      ...
      const r = await post('/api/ai/chat', {messages:[{role:'user',content:'Reply with just: \"DotServe AI connected ✓\"'}]});
...
function monitoringPage() {
  return {
    ...
    async init() { await this.load(); setInterval(()=>this.load(), 5000); document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="monitoring") this.load(); }); },
...
function bandwidthPage() {
  return {
    ...
    async init() {
      ...
      document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="bandwidth") { this.loadSummary(); this.loadDomains(); } });
    },
...
function alertsPage() {
  return {
    config: {}, history: [], metrics: {}, loading: true,
    async init() { await this.load(); window.addEventListener("vp:page", (e) => { if(e.detail==="alerts") this.load(); }); },
    async load() {
      this.loading = true;
      const r = await get('/api/alerts');
      if (r.ok) { this.config = r.config || {}; this.history = r.history || []; this.metrics = r.metrics || {}; }
      this.loading = false;
    },
    async save() {
      const r = await put('/api/alerts/config', this.config);
      toast(r.ok ? 'Alert settings saved' : (r.error || 'Save failed'), r.ok ? 'success' : 'error');
      if (r.ok) this.config = r.config;
    },
    async evaluate() {
      const r = await post('/api/alerts/evaluate', {});
      toast(r.triggered?.length ? `${r.triggered.length} alert(s) triggered` : 'No thresholds exceeded', r.triggered?.length ? 'error' : 'success');
      await this.load();
    },
  };
}
...
// --- DISK USAGE -----------------------------------------------------------------
function diskUsagePage() {
  return {
    path: '/www/wwwroot', items: [], duplicates: [], loading: false,
    async init() { await this.load(); window.addEventListener("vp:page", (e) => { if(e.detail==="disk-usage") this.load(); }); },
    async load() {
      this.loading = true;
      const r = await get('/api/disk-usage/tree?path=' + encodeURIComponent(this.path) + '&depth=1');
      if (r.ok) { this.path = r.path; this.items = r.items || []; } else toast(r.error || 'Disk scan failed', 'error');
      this.loading = false;
    },
    async findDuplicates() {
      toast('Scanning for duplicate files...', 'info');
      const r = await get('/api/disk-usage/duplicates?path=' + encodeURIComponent(this.path));
      if (r.ok) this.duplicates = r.groups || []; else toast(r.error || 'Duplicate scan failed', 'error');
    },
    async deleteFile(path) {
      if (!confirm('Delete this file?')) return;
      const r = await post('/api/disk-usage/delete', {path});
      toast(r.ok ? 'File deleted' : (r.error || 'Delete failed'), r.ok ? 'success' : 'error');
      await this.load();
    },
    fmtBytes,
  };
...
function webshellPage() {
  return {
    path: '/www/wwwroot', findings: [], scanned: 0, loading: false,
    async scan() {
      this.loading = true;
      const r = await post('/api/webshell/scan', {path: this.path});
      if (r.ok) { this.findings = r.findings || []; this.scanned = r.scanned || 0; this.path = r.root || this.path; }
      else toast(r.error || 'Scan failed', 'error');
      this.loading = false;
    },
    async quarantine(path) {
      const r = await post('/api/webshell/quarantine', {path});
      toast(r.ok ? 'File quarantined' : (r.error || 'Quarantine failed'), r.ok ? 'success' : 'error');
      if (r.ok) await this.scan();
    },
    async remove(path) {
      if (!confirm('Permanently delete this file?')) return;
      const r = await post('/api/webshell/delete', {path});
      toast(r.ok ? 'File deleted' : (r.error || 'Delete failed'), r.ok ? 'success' : 'error');
      if (r.ok) await this.scan();
    },
  };
...
function onboardingPage() {
  return {
    data: {steps: [], server: {}, email: {}, security: {}, completed: false},
    ...
    async save(completed=false) {
      this.data.completed = completed || this.data.completed;
      const r = await put('/api/onboarding', this.data);
      toast(r.ok ? 'Onboarding saved' : (r.error || 'Save failed'), r.ok ? 'success' : 'error');
      if (r.ok) this.data = r;
    },
  };
}
...
function securityPage() {
  return {
    ...
    async init() {
      ...
      await Promise.all([this.loadScore(), this.loadSSH()]); document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="security") { this.loadScore(); this.loadSSH(); } });
...
const DOCKER_CATALOG = [
  {id:'nginx', icon:'🌐', name:'Nginx', hardened:false, image:'nginx', tag:'latest', cat:'Web Server',
   ...
   ports:[{host:'8080', container:'80'}], envs:[], volumes:[{host:'/opt/dotserve/docker-data/nginx', container:'/usr/share/nginx/html'}],
  ...
  {id:'httpd', icon:'🪶', name:'Apache HTTPD', hardened:false, image:'httpd', tag:'latest', cat:'Web Server',
   ...
   ports:[{host:'8081', container:'80'}], envs:[], volumes:[{host:'/opt/dotserve/docker-data/httpd', container:'/usr/local/apache2/htdocs'}],
  ...
  {id:'mysql', icon:'🐬', name:'MySQL', hardened:false, image:'mysql', tag:'8.0', cat:'Database',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/mysql', container:'/var/lib/mysql'}],
  ...
  {id:'postgres', icon:'🐘', name:'PostgreSQL', hardened:false, image:'postgres', tag:'16', cat:'Database',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/postgres', container:'/var/lib/postgresql/data'}],
  ...
  {id:'mariadb', icon:'🦭', name:'MariaDB', hardened:false, image:'mariadb', tag:'11', cat:'Database',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/mariadb', container:'/var/lib/mysql'}],
  ...
  {id:'mongo', icon:'🍃', name:'MongoDB', hardened:false, image:'mongo', tag:'7', cat:'Database',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/mongo', container:'/data/db'}],
  ...
  {id:'redis', icon:'⚡', name:'Redis', hardened:false, image:'redis', tag:'7-alpine', cat:'Cache',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/redis', container:'/data'}],
  ...
  {id:'wordpress', icon:'📝', name:'WordPress', hardened:false, image:'wordpress', tag:'latest', cat:'CMS',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/wordpress', container:'/var/www/html'}],
  ...
  {id:'portainer', icon:'🐳', name:'Portainer', hardened:false, image:'portainer/portainer-ce', tag:'latest', cat:'Management',
   ...
   volumes:[
     ...
     {host:'/opt/dotserve/docker-data/portainer', container:'/data'},
   ],
  ...
  {id:'grafana', icon:'📊', name:'Grafana', hardened:false, image:'grafana/grafana', tag:'latest', cat:'Monitoring',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/grafana', container:'/var/lib/grafana'}],
  ...
  {id:'prometheus', icon:'🔥', name:'Prometheus', hardened:false, image:'prom/prometheus', tag:'latest', cat:'Monitoring',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/prometheus', container:'/prometheus'}],
  ...
  {id:'rabbitmq', icon:'🐰', name:'RabbitMQ', hardened:false, image:'rabbitmq', tag:'3-management', cat:'Messaging',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/rabbitmq', container:'/var/lib/rabbitmq'}],
  ...
  {id:'vaultwarden', icon:'🔐', name:'Vaultwarden', hardened:false, image:'vaultwarden/server', tag:'latest', cat:'Security',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/vaultwarden', container:'/data'}],
  ...
  {id:'uptime-kuma', icon:'🟢', name:'Uptime Kuma', hardened:false, image:'louislam/uptime-kuma', tag:'1', cat:'Monitoring',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/uptime-kuma', container:'/app/data'}],
  ...
  {id:'traefik', icon:'🔀', name:'Traefik', hardened:false, image:'traefik', tag:'v3.0', cat:'Web Server',
   ...
   volumes:[{host:'/var/run/docker.sock', container:'/var/run/docker.sock'},{host:'/opt/dotserve/docker-data/traefik', container:'/etc/traefik'}],
  ...
  {id:'nginx-proxy-manager', icon:'🔁', name:'Nginx Proxy Manager', hardened:false, image:'jc21/nginx-proxy-manager', tag:'latest', cat:'Web Server',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/npm/data', container:'/data'},{host:'/opt/dotserve/docker-data/npm/letsencrypt', container:'/etc/letsencrypt'}],
  ...
  {id:'wireguard', icon:'🔒', name:'WireGuard VPN', hardened:false, image:'linuxserver/wireguard', tag:'latest', cat:'Networking',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/wireguard', container:'/config'}],
  ...
  {id:'pihole', icon:'🕳', name:'Pi-hole', hardened:false, image:'pihole/pihole', tag:'latest', cat:'Networking',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/pihole/etc-pihole', container:'/etc/pihole'},{host:'/opt/dotserve/docker-data/pihole/etc-dnsmasq', container:'/etc/dnsmasq.d'}],
  ...
  {id:'netdata', icon:'📈', name:'Netdata', hardened:false, image:'netdata/netdata', tag:'latest', cat:'Monitoring',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/netdata', container:'/etc/netdata'}],
  ...
  {id:'gitea', icon:'🍵', name:'Gitea', hardened:false, image:'gitea/gitea', tag:'latest', cat:'Dev Tools',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/gitea', container:'/data'}],
  ...
  {id:'code-server', icon:'💻', name:'Code Server', hardened:false, image:'codercom/code-server', tag:'latest', cat:'Dev Tools',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/code-server', container:'/home/coder/project'}],
  ...
  {id:'n8n', icon:'🔗', name:'n8n', hardened:false, image:'n8nio/n8n', tag:'latest', cat:'Automation',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/n8n', container:'/home/node/.n8n'}],
  ...
  {id:'nextcloud', icon:'☁', name:'Nextcloud', hardened:false, image:'nextcloud', tag:'latest', cat:'File Storage',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/nextcloud', container:'/var/www/html'}],
  ...
  {id:'minio', icon:'🪣', name:'MinIO', hardened:false, image:'minio/minio', tag:'latest', cat:'File Storage',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/minio', container:'/data'}],
  ...
  {id:'meilisearch', icon:'🔍', name:'Meilisearch', hardened:false, image:'getmeili/meilisearch', tag:'latest', cat:'Search',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/meilisearch', container:'/meili_data'}],
  ...
  {id:'influxdb', icon:'📉', name:'InfluxDB', hardened:false, image:'influxdb', tag:'2', cat:'Database',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/influxdb', container:'/var/lib/influxdb2'}],
  ...
  {id:'ghost', icon:'👻', name:'Ghost', hardened:false, image:'ghost', tag:'latest', cat:'CMS',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/ghost', container:'/var/lib/ghost/content'}],
  ...
  {id:'jellyfin', icon:'🎬', name:'Jellyfin', hardened:false, image:'jellyfin/jellyfin', tag:'latest', cat:'Media Server',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/jellyfin/config', container:'/config'},{host:'/opt/dotserve/docker-data/jellyfin/media', container:'/media'}],
  ...
  {id:'duplicati', icon:'🗄', name:'Duplicati', hardened:false, image:'duplicati/duplicati', tag:'latest', cat:'Backup',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/duplicati/config', container:'/config'},{host:'/opt/dotserve/backups', container:'/source'}],
  ...
  {id:'filebrowser', icon:'📁', name:'File Browser', hardened:false, image:'filebrowser/filebrowser', tag:'latest', cat:'File Storage',
   ...
   volumes:[{host:'/opt/dotserve', container:'/srv'}],
  ...
  {id:'node', icon:'🟢', name:'Node.js', hardened:false, image:'node', tag:'lts-alpine', cat:'Runtime',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/node-app', container:'/app'}],
  ...
  {id:'python', icon:'🐍', name:'Python', hardened:false, image:'python', tag:'3.12-slim', cat:'Runtime',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/python-app', container:'/app'}],
  ...
  {id:'ollama', icon:'🦙', name:'Ollama', hardened:false, image:'ollama/ollama', tag:'latest', cat:'AI / LLM',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/ollama', container:'/root/.ollama'}],
  ...
  {id:'open-webui', icon:'🤖', name:'Open WebUI', hardened:false, image:'ghcr.io/open-webui/open-webui', tag:'main', cat:'AI / LLM',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/open-webui', container:'/app/backend/data'}],
  ...
  {id:'qdrant', icon:'🧭', name:'Qdrant', hardened:false, image:'qdrant/qdrant', tag:'latest', cat:'AI / LLM',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/qdrant', container:'/qdrant/storage'}],
  ...
  {id:'localai', icon:'🧠', name:'LocalAI', hardened:false, image:'localai/localai', tag:'latest-cpu', cat:'AI / LLM',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/localai/models', container:'/models'}],
  ...
  {id:'flowise', icon:'🌊', name:'Flowise', hardened:false, image:'flowiseai/flowise', tag:'latest', cat:'AI / LLM',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/flowise', container:'/root/.flowise'}],
  ...
  {id:'mosquitto', icon:'📡', name:'Eclipse Mosquitto', hardened:false, image:'eclipse-mosquitto', tag:'latest', cat:'IoT / Home Automation',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/mosquitto/config', container:'/mosquitto/config'},{host:'/opt/dotserve/docker-data/mosquitto/data', container:'/mosquitto/data'}],
  ...
  {id:'node-red', icon:'🔴', name:'Node-RED', hardened:false, image:'nodered/node-red', tag:'latest', cat:'IoT / Home Automation',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/node-red', container:'/data'}],
  ...
  {id:'home-assistant', icon:'🏠', name:'Home Assistant', hardened:false, image:'ghcr.io/home-assistant/home-assistant', tag:'stable', cat:'IoT / Home Automation',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/home-assistant', container:'/config'}],
  ...
  {id:'jenkins', icon:'⚙', name:'Jenkins', hardened:false, image:'jenkins/jenkins', tag:'lts', cat:'Dev Tools',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/jenkins', container:'/var/jenkins_home'}],
  ...
  {id:'verdaccio', icon:'📦', name:'Verdaccio', hardened:false, image:'verdaccio/verdaccio', tag:'latest', cat:'Dev Tools',
   ...
   volumes:[{host:'/opt/dotserve/docker-data/verdaccio', container:'/verdaccio/storage'}],
...
function dockerPage() {
  return {
    ...
    async init() {
      ...
      this.loading = false;
      document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="docker") { this.loadStatus(); if(this.status.running){ this.loadContainers(); this.loadImages(); } } });
...
function cronPage() {
  return {
    ...
    async init() {
      ...
      await this.load();
      document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="cron") this.load(); });
    },
...
function caddyPage() {
  return {
    ...
    async init() { await Promise.all([this.loadStatus(), this.loadSites()]); document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="caddy") { this.loadStatus(); this.loadSites(); } }); },
...
function cdnPage() {
  return {
    ...
    async init() {
      ...
      const ws = await get('/api/websites');
      if (ws.ok) this.sites=ws.sites||[];
      document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="cdn") this.load(); });
    },
    ...
    async checkForUpdates() {
      ...
      try {
        ...
        try {
          ...
          if (app) {
            app.updateModal.current   = r.current   || 'v1.0.0';
            app.updateModal.latest    = r.latest    || r.current || 'v1.0.0';
            app.updateModal.name      = r.name      || 'DotServe';
...
function logsPage() {
  return {
    sources: [], source: 'dotserve', search: '', lines: 200,
    ...
    async init() {
      ...
      document.addEventListener("dotserve-logged-in", () => { this.init(); }); window.addEventListener("vp:page", (e) => { if(e.detail==="logs") this.load(); });
    },
    async load(silent) {
