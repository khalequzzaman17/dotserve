  ...
  return {
    ...
    logout() {
      fetch('/api/auth/logout', {method:'POST'})
        .then(() => { window.dotserveAuthenticated=false; this.loggedIn=false; this.loginUser=''; this.loginPass=''; this.loginErr=''; });
    },
  ...
  return {
    ...
    async init() {
      document.addEventListener("dotserve-logged-in", () => {
        if ((location.hash || '#dashboard').replace('#', '') === 'disk-usage') this.load();
      });
      window.addEventListener("vp:page", (e) => { if(e.detail==="disk-usage") this.load(); });
    },
    async load() {
      if (!window.dotserveAuthenticated) return;
      this.loading = true;
      const r = await get('/api/disk-usage/tree?path=' + encodeURIComponent(this.path) + '&depth=1');
      if (r.ok) { this.path = r.path; this.items = r.items || []; }
      else if (!r.unauthorized) toast(r.error || 'Disk scan failed', 'error');
      this.loading = false;
    },
